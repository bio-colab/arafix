...
### التشخيص المتقدم، fast-path التخطيط، والجداول المعقدة — 2026-08-12

هذه التغييرات على فرع `feat/advanced-diagnostics` مبنية على `main` عند الالتزام `19168ed7b8d9daaba431c4662ce27edeb631475c`، ومرفوعة في [PR #9](https://github.com/bio-colab/arafix/pull/9). لم تُضمّن بيانات corpus الخارجية أو ملفات `pstats` أو نتائج القياس المؤقتة في الالتزام.

#### fast-path لإعادة استخدام تخطيط الصفحة

كان مستخرج PyMuPDF يبني `PageLayout` ليكوّن `RawPage.text`، ثم كان `pipeline._extract_one_page` يعيد التحليل الكامل للجليفات نفسها. أُضيف مسار محافظ يعيد استخدام التخطيط الموجود عندما تكون إعدادات `LayoutConfig` افتراضية ولم تحدث إعادة بناء CMap؛ أما بعد استرداد CMap أو مع إعدادات تخطيط مخصصة فيستمر التحليل الكامل. لا توجد ذاكرة تخطيط عابرة للصفحات ولا تخمين هندسي جديد.

| ملف PDF بسيط، 2000 صفحة | قبل | بعد | الأثر |
|---|---:|---:|---:|
| الزمن الكلي | 47.436694 s | **42.706051 s** | **خفض 9.9800%** |
| throughput | 42.161 صفحة/s | **46.832 صفحة/s** | **زيادة 11.0700%** |
| SHA-256 للنص | `92a6b41f30ed851eea5608474e4583dc75e32f1b4cee46746a234d3199179ae1` | نفس البصمة | تطابق حرفي |
| استدعاءات `analyze_layout` تحت cProfile | 4000 | **2000** | أزيلت إعادة الحساب |

#### Diacritic Trace Log — تشخيص بلا إصلاح غير مثبت

أضيفت أداة `scripts/diacritic_trace.py` لتتبع علامات التشكيل من `get_texttrace()` إلى الجليفات والتخطيط ومرحلة Hygiene والمخرج النهائي. في ملف الدستور بقي عدد العلامات ثابتاً في المثال الممثل: `trace=13`, `extracted=13`, `layout=13`, `hygiene=13`, `final=13`. لذلك فالمشكلة المرصودة هي إرفاق علامة بقاعدة غير صحيحة في مرحلة الاستخراج، وليست فقداناً لاحقاً.

في حالة `جمهورٌي` كان origin الضمة أقرب إلى الراء هندسياً، بينما يبيّن تداخل الرسم أنها مرتبطة بالياء في النص المرجعي. جُرّبت قاعدة bbox واسعة، فكسرت اختبارين وخفضت الثقة إلى `0.350`، ثم جُرّبت قاعدة exact-next مع تغطية bbox بنسبة 90%؛ اجتازت **301/301** اختباراً لكنها أبقت CER/WER عند **3.09% / 17.21%**. سُحبت التجربتان ولم يتغير `pymupdf_extractor.py`، التزاماً بقاعدة عدم إضافة تعقيد بلا مكسب مقاس.

#### corpus دلالي موثق بـAraBERT

أضيفت `benchmarks/semantic_eval.py` و`scripts/build_arabic_gold_corpus.py` كأدوات تقييم اختيارية لا تدخل في تبعيات النواة. جُمعت 50 صفحة حقيقية من ويكيبيديا العربية عبر بحث موضوعي حتمي مع pagination، ثم استخراج صريح لكل صفحة لأن MediaWiki يفرض `exlimit=1` في طلب النص الكامل. استُخدم حد أدنى قدره 1000 محرف مستخرج، وحُفظت metadata الخاصة بالصفحة والإصدار ووقت الجلب في corpus محلي غير ملتزم به.

على manifest من 50 زوجاً، كانت الفرضيات ناتجة من `arafix.repair_text` فعلياً، لا من نسخ المرجع. باستخدام `aubmindlab/bert-base-arabert` و`max_length=256` بلغ المتوسط **0.9997732806205749**، والحد الأدنى **0.9963199496269226**، والحد الأقصى **1.000000238418579**. هذا قياس حفظ دلالي لمرحلة الإصلاح على نصوص عربية حقيقية؛ الأداة الحالية تقص كل وثيقة إلى أول 256 token، ولذلك لا يُقدّم الرقم بوصفه تقييماً كاملاً لاستخراج PDF أو تغطية كل محارف الوثيقة.

#### إجهاد الجداول المعقدة

أضيف اختبار regression صغير للعنوان المدمج، وأُجريت قياسات خارجية على ثلاث حالات ReportLab مؤقتة: خلايا/عنوان مدمج، جدول بلا حدود مرئية، وجدول يمتد عبر صفحتين. أُضيف إلى `_detect_tables_in_lines` احتواء singleton سابق فقط إذا كان ملاصقاً رأسياً، واقعاً أفقياً داخل نطاق grid، وكان الجدول العادي قد اجتاز شروط الحد الأدنى.

| الحالة | F1 قبل | F1 بعد | النتيجة البنيوية |
|---|---:|---:|---|
| عنوان/خلية مدمجة | 0.962963 | **1.000000** | جدول واحد، 6 صفوف × 3 أعمدة |
| حدود غير مرئية | 1.000000 | **1.000000** | جدول واحد، 5 صفوف × 4 أعمدة |
| جدول متعدد الصفحات | 1.000000 | **1.000000** | جدول لكل صفحة، 108 خلية مستردة |

#### توافق بوابة CI

أُعيد ترتيب imports ولفّ الأسطر الطويلة في الملفات التي يفحصها CI فقط، من دون تغيير منطق وقت التشغيل أو معايير الاختبارات. وثُبّت Ruff في dev extra عند `>=0.4,<0.5` حتى لا يتحول اختلاف lint بين الإصدارات إلى ضوضاء غير مرتبطة بهذه الدفعة؛ يظل `ruff check src tests` أخضر على Ruff 0.4.10.

#### بوابة القبول

اجتازت النسخة بعد التغيير المقبول واختبار regression الجديد **302/302 اختباراً**، ومرّ `git diff --check`. ملفات PDF التجريبية، corpus ويكيبيديا، manifests، `pstats`، وملفات نتائج benchmark بقيت خارج الالتزام. لا يوجد إصلاح إنتاجي للتشكيل في هذه الدفعة؛ الدليل الحالي يبرر التشخيص فقط.
