...
المبدأ الثالث — الخط الفاصل الرسمي للمشروع (مثبَّت في
tests/hardening/test_h15_mission_boundary.py):

  ٣. **استرجاع الترميز ≠ تصحيح لغوي.** arafix يستعيد ما كان في
     الترميز (انعكاس، أشكال عرض، موجيبيك، CMap) ولا يصحّح ما كتبه
     المؤلف. «المجالت» قد تكون خطأ استخراجٍ لرباط لا-ألف وقد تكون النص
     الأصلي نفسه؛ ولذلك لا تعبر أي قاعدةٌ حدودَ فئة العيب المغلقة إلا
     بشاهدٍ من فئتها ومعجمٍ مُثبِت وتدقيقٍ مسجَّل — ولا وجود لمسار
     «تصحيح إملائي عام» ولا سيُنشأ.
"""
