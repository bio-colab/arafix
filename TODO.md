...
| **P2** | **هندسة المخرجات المنظمة وتوفير التوكنز لـ LLMs** | استخراج Markdown الهيكلي، كائن TableResult مع تصدير CSV/Pandas/Dict، تخفيض توكنز BPE حتى 45% عبر `doc.to_llm_text()`، ومداخل السطر الواحد `read_markdown` و `read_llm`. | **منجز ومثبت بنجاح (1.2.0)** |
| **P3** | **توسيع مدونة المنتجين (Producer Corpus)** | جمع عينات حقيقية مصنفة من Word, InDesign, XeLaTeX, Skia وبناء بوابات نوعية لكل منتج. | مستمر (P0 و P1 منفذان) |
| **P4** | **توصيل GlyphEvidence داخل `context.repair()`** | ربط أدلة الجليفات المحصنة داخل محرك دمج الأدلة في حالات الخطوط المبتورة. | جاهز للربط |
