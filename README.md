...
النص الخام
    ...
    │
    ├─ ١أ normalize.py ◄── الأشكال المفردة وحدها. الرباطُ ذرّةٌ لا تُشقّ.
    ├─ ٢ order.py      ◄── بصريّ ← منطقيّ، بحماية الأرقام واللاتينية
    ├─ ١ب normalize.py ◄── فكّ الرباطات — الآن فقط، بعد استقرار الترتيب
...
**٥ب. الكاشف يقرأ طبقتين، لأن التطبيع يفقأ عيناً وهو يفتح أخرى.**
...
**٥ج. هويّة الوصل برهانٌ لا أمارة.**
...
```bash
pytest                      # ٦٩ اختباراً + ١٠ doctests
